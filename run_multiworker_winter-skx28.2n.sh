...
HOROVOD_FUSION_THRESHOLD=524288000

command="$MPI -np $NUM_WORKERS --hostfile $HOSTFILE --map-by socket \
  -x KMP_AFFINITY=$KMP_AFFINITY -x KMP_BLOCKTIME=0 -x LD_LIBRARY_PATH=$LD_LIBRARY_PATH \
  -x HOROVOD_FUSION_THRESHOLD=$HOROVOD_FUSION_THRESHOLD -x PATH=$PATH \
