...
#OMP_NUM_THREADS=$OMP_NUM_THREADS KMP_BLOCKTIME=0 KMP_AFFINITY="compact,1,0,granularity=fine" taskset -c 0-19,40-59 numactl -m 0 python run_pretraining_horovod.py \
KMP_BLOCKTIME=0 KMP_AFFINITY="compact,1,0,granularity=fine" python run_pretraining_horovod.py \
  --input_file=$DATA_DIR \
  --output_dir=/data/01/kushal/bert/pretraining_output_mkl \
  --do_train=True \
